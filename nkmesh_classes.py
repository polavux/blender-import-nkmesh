...
def unpack_next_string(data) -> str:
    ...
    return unpack_next(f"{length}s", data)[0].decode("ascii")
...
class Nkmesh:
    ...
    def from_file(self, file_path: str):
        ...
        return self(version, nodes, meshes)
    ...
    class NkmeshMesh:
        ...
        def from_stream(self, data):
            ...
            unknown_4_bytes = unpack_next_single_int(data)
        ...
        class NkmeshVertexColorSet:
            @classmethod
            def from_stream(self, data):
                # colors use run-length encoding
                num_colors = unpack_next_single_int(data)
                num_colors2 = unpack_next_single_int(data)
    ...
    class NkmeshNode:
        ...
        def from_stream(self, data):
            ...
            parent_node_index, mesh_index, grandparent_node_index, num_child_nodes, unknown_int = unpack_next("<iiiII", data)
            child_nodes = unpack_next(f"<{num_child_nodes}I", data)
            unknown_transforms, unknown_4_bytes, offset_next_node = unpack_next("<41f", data), unpack_next_single_int(data), unpack_next_single_int(data)
            
            print(f"Found node |name: {name:<50}|type: {node_type:<50} {unknown_int} {unknown_4_bytes}")

            return self(
                name, node_type,
                parent_node_index, mesh_index, grandparent_node_index, child_nodes, unknown_int,
                 unknown_transforms, unknown_4_bytes, offset_next_node
            )
        ...
        unknown_int: int
        unknown_transforms: tuple[float]
        unknown_4_bytes: int
